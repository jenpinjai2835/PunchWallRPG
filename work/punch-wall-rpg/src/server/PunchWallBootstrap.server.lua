		...
		if rootPart and humanoid and humanoid.Health > 0 then
			...
			for _, block in ipairs(workspace:GetPartBoundsInBox(boxCFrame, boxSize, overlap)) do
				if block:GetAttribute("IsDepthBlock") and not block:GetAttribute("Broken") then
					if not block:GetAttribute("StructuralDetached") and block.Anchored and depthPunch.DropStructural(block, player, true) then
						ejected += 1
					elseif block:GetAttribute("StructuralDetached") and depthPunch.ClearCharactersFromStructuralBlock(block) > 0 then
						ejected += 1
					end
				end
			end
