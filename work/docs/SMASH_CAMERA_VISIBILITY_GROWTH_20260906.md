...
### Native R2 setup timeout and verified cleanup API

The preserved `smash-premium-pet-narrow-profile-r2-20260906.json` confirms successful cleared-state detection and creation of the exact 637 × 654 preset. It then timed out waiting 35 seconds for the `start_stop_play` response, before seeding or frame observation. Cleanup successfully stopped Play and verified Server/Client absence. Device cleanup failed because the initially proposed `ClearDeviceAsync()` does **not** exist on the installed service. The Coordinator owns removal of R2's uniquely named leftover preset; subsequent runs cannot overwrite R2 or silently claim its cleanup passed.

The tool now uses the existing capture and pet-flow cleanup API, `StopSimulationAsync()`, followed by cleared-state readback and removal of only its own preset through `RemoveDeviceAsync()`. The device mock exposes these verified methods and deliberately omits the unsupported member; an executed negative control reproduces the erroneous `ClearDeviceAsync()` call and proves it fails without removing a preset. Both start and stop Play now receive the ordinary **60-second MCP response timeout**. This changes setup/cleanup allowance, not the six-second native sample window or any geometry/timing completeness predicate.

The Coordinator reported concurrent static compilation during R2 startup. There were no measured frames, so the timeout is not evidence of game frame performance. The next run must use a new evidence leaf with exclusive Studio/CPU measurement and no concurrent aggregate compilation. Updated offline results: **30 Node guards, nine compiled snippets, 23 metric controls, 11 exact seed controls, 14 exact device-lifecycle controls, and the same five rejected weakening controls**. Syntax and diff checks pass. Native frame measurement remains pending.
