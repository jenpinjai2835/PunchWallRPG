...
const checks = [
  ...
  ["rebirth_uses_icon_led_real_requirements", client.includes('"WallLevelRequirement"') && client.includes('"CoinsRequirement"') && client.includes('latestStats.RebirthRequiredLevel') && client.includes('latestStats.RebirthRequiredCoins') && client.includes('createThemeIcon(levelCard, "Wall"') && client.includes('createThemeIcon(coinCard, "RebirthCoin"')],
  ...
  ["generated_assets_are_versioned", ["wall-level.png", "coins.png", "coins-v2.png", "rebirth-core.png", "README.md"].every((name) => fs.existsSync(path.join(root, "assets", "generated", "rebirth-ui-v1", name)))],
  ["approved_rebirth_coin_is_configured", client.includes('createThemeIcon(coinCard, "RebirthCoin"') && read("punch-wall-rpg", "src", "shared", "GameConfig.lua").includes('RebirthCoin = "rbxassetid://135091225093305"') && flowText.includes("ThemeIcon')=='RebirthCoin") && flowText.includes("ThemeIconSource')=='StandaloneAsset")],
