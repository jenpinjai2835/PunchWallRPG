...
const checks = [
  ...
  ["base_power_authority_precedes_mutation", trainFunction.includes('local power = statValue(player, "Power", 0)') && trainFunction.indexOf("if power < config.minPower") < trainFunction.indexOf("setActiveTrainingStation(player, config)") && !trainFunction.includes("EffectivePower")],
  ...
  ["world_reset_preserves_eligible_training", server.includes("if activeStation and statValue(player, \"Power\", 0) >= activeStation.minPower then") && server.includes('player:SetAttribute("TrainingTickAnchor", workspace:GetServerTimeNow())') && server.includes("stopTraining(player, \"world_reset_ineligible\")")],
  ...
  ["interaction_identity_and_radius_are_server_validated", server.includes("TRAINING_INTERACTION_DISTANCE = 18") && server.includes("if requested and requested ~= config then config = nil end") && client.includes('target = gui:GetAttribute("ContextualActionTarget")')],
  ["runtime_covers_boundaries_rates_switch_reset_and_stop", hasLabel("threshold minus one rejects base Power even with huge multipliers and never pays later") && hasLabel("each exact threshold starts without free payout then earns its exact one-second rate") && flowText.includes("normalized==case[3]") && flowText.includes("after.lastTickBatch==ticks") && hasLabel("same station is idempotent and active switch pays only the new station rate") && hasLabel("world reset preserves and reanchors active training without a duplicate payout") && hasLabel("stop clears movement lock and no server tick leaks afterward")],
