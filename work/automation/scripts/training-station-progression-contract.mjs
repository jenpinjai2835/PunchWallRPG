...
const checks = [
  ...
  ["runtime_covers_boundaries_rates_switch_reset_and_stop", hasLabel("threshold minus one rejects displayed effective Power and never pays later") && hasLabel("each exact threshold starts without free payout then earns its exact one-second rate") && flowText.includes("before.qualificationPower<case[3]") && flowText.includes("normalized==case[3]") && flowText.includes("after.lastTickBatch==ticks") && hasLabel("same station is idempotent and active switch pays only the new station rate") && hasLabel("world reset preserves and reanchors active training without a duplicate payout") && hasLabel("stop clears movement lock and no server tick leaks afterward")],
